...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            corners = tuple((x, y) for (x, y) in corners if x != nextx or y != nexty)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if not tg:
        return 0
    return min([getDistance(pos, e) for e in permu(tg)])




def getDistance(cur, path, distc = util.manhattanDistance):
    d = 0
    for c in path:
        d += distc(cur, c)
        cur = c
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    d = 0
    for item in foodGrid:
        k = (position, item)
        if k not in cache:
            cache[k] = mazeDistance(position, item, problem.startingGameState)
        d2 = cache[k]
        if d2 > d:
            d = d2
    return d
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        x,y = state
        self_food_list = self.food.asList()
        return (x, y) in self_food_list
